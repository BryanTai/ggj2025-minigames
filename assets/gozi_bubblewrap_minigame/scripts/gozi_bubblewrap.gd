...
func _process_preparing_state(delta: float) -> void:
	$"Bubbles/3/AnimatedSprite2D".animation = "whole1"
	$"Bubbles/8/AnimatedSprite2D".animation = "whole1"
	$"Bubbles/19/AnimatedSprite2D".animation = "whole1"
	$"Bubbles/20/AnimatedSprite2D".animation = "whole1"
	$"Bubbles/23/AnimatedSprite2D".animation = "whole1"

# manually check every frame if all of the bubbles are popped LOLOLOL
func _process_playing_state(delta: float) -> void:
	if $"Bubbles/3/AnimatedSprite2D".animation == "popped2" and $"Bubbles/8/AnimatedSprite2D".animation == "popped2" and $"Bubbles/19/AnimatedSprite2D".animation == "popped2" and $"Bubbles/20/AnimatedSprite2D".animation == "popped2" and $"Bubbles/23/AnimatedSprite2D".animation == "popped2":
		trigger_game_win()
